...
import {bookingHistoryColumns,completedRevenue,type Booking} from "@/lib/booking-history";
...
export async function GET(req:Request){
  try{
    ...
    const [bookings,customer]=await Promise.all([
      db().prepare(`SELECT ${bookingHistoryColumns} FROM bookings WHERE phone=? ORDER BY date DESC,start DESC`).bind(phone).all<Booking>(),
    ...
    const totalSpent=completedRevenue(history);
    const lastService=history.length>0?history[0].serviceName:null;
