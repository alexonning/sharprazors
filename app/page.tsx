...
{customerState!=="unknown"&&<div className="confirmation-summary"><span>SEU AGENDAMENTO</span><strong>{selectedNames}</strong><p>{prettyDate} às {slot===null?"—":time(slot)} · {selectedDuration} min · {barbersList.find(b=>b.id===barber)?.name||"Qualquer disponível"}</p></div>}
...
{error&&step>0&&<div className="error" role="alert">{error}{step===1&&<button onClick={()=>setReload(x=>x+1)}>Tentar novamente</button>}{blocked&&site?.whatsapp&&<a className="error-whatsapp" href={whatsappUrl(site.whatsapp)+"?text="+encodeURIComponent("Olá! Tentei agendar um horário pelo site e não consegui. Podem me ajudar?")} target="_blank" rel="noreferrer"><WhatsApp size={17}/>Falar no WhatsApp · {formatPhone(site.whatsapp)}<ArrowRight size={15}/></a>}</div>}
...
<aside><section className="brand-card"><img src="/logo.png" alt="Logo Sharp Razors"/><p className="eyebrow">BARBEARIA</p><h2>NA RÉGUA.<br/>NO SEU TEMPO.</h2><div className="brand-divider"/><div className="appointment-summary"><span>SEU AGENDAMENTO</span>{selected.length>0&&<><div><Scissors size={17}/><strong>{selectedNames}</strong></div><div><Clock size={17}/>{selectedDuration} minutos · {barbersList.find(b=>b.id===barber)?.name||"Qualquer disponível"}</div></>}{slot!==null&&<div><CalendarDays size={17}/>{prettyDate}, {time(slot)}</div>}</div></section>
