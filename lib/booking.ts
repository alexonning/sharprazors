...
export type SiteConfig={whatsapp:string,phone:string,hours:DayHours[],services:Service[],bookingWindowDays:number,minAdvanceMinutes:number};
...
export function lastBookableDate(days=DEFAULT_BOOKING_WINDOW_DAYS){const value=new Date(today()+"T12:00:00Z");value.setUTCDate(value.getUTCDate()+Math.max(1,days)-1);return value.toISOString().slice(0,10)}
export function validDate(date:string,days=DEFAULT_BOOKING_WINDOW_DAYS){return /^\d{4}-\d{2}-\d{2}$/.test(date)&&!isNaN(Date.parse(date))&&new Date(date+"T12:00:00Z").toISOString().slice(0,10)===date&&date>=today()&&date<=lastBookableDate(days)}
...
export function slots(date:string,duration:number,hours:DayHours[],blocks:{start:number,end:number}[]=[],minAdvanceMinutes=DEFAULT_MIN_ADVANCE_MINUTES,bookingWindowDays=DEFAULT_BOOKING_WINDOW_DAYS){if(!validDate(date,bookingWindowDays))return [];const day=hours[new Date(date+"T12:00:00Z").getUTCDay()];if(!day||day.closed)return [];const result=new Set<number>();const earliestStart=Date.now()+minAdvanceMinutes*60000;for(const [start,end] of day.periods)for(let t=start;t+duration<=end;t+=5){if(!blocks.some(b=>t<b.end&&t+duration>b.start)&&new Date(date+"T"+time(t)+":00-03:00").getTime()>=earliestStart)result.add(t)}return [...result].sort((a,b)=>a-b)}
