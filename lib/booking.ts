...
export function slots(date:string,duration:number,hours:DayHours[],blocks:{start:number,end:number}[]=[]){if(!validDate(date))return [];const day=hours[new Date(date+"T12:00:00Z").getUTCDay()];if(!day||day.closed)return [];const result=new Set<number>();for(const [start,end] of day.periods)for(let t=start;t+duration<=end;t+=5){if(!blocks.some(b=>t<b.end&&t+duration>b.start)&&new Date(date+"T"+time(t)+":00-03:00").getTime()>Date.now())result.add(t)}return [...result].sort((a,b)=>a-b)}
